...
NebulaX Track Access Controller: Full end-to-end workflow featuring CP-SAT global optimisation,
rule verification, score diagnostics, interactive milestone timeline, track possession heatmaps,
dependency inspection, and verified submission package export.
"""
...
# Custom NebulaX Styling & Palette
st.markdown(
    """
    <style>
    .main-title {
    ...
    .metric-label { font-size: 0.85rem; color: #475569; font-weight: 500; }
    .metric-value { font-size: 1.6rem; font-weight: 700; color: #1E3A8A; }
    .stTabs [data-baseweb="tab-list"] { gap: 8px; }
    .stTabs [data-baseweb="tab"] { height: 42px; font-weight: 600; border-radius: 6px; }
    </style>
    """,
...
# Title Header
st.markdown('<div class="main-title">🚇 Railway Track Access Optimisation Engine</div>', unsafe_allow_html=True)
st.markdown(
    """
    <div class="sub-title">
    <strong>Deterministic, Globally Optimal, Zero-Hard-Violation Access Scheduler</strong> for Line Alpha & Line Beta engineering horizons.<br>
    Rigidly satisfies track safety geometry (buffers, opposite-bound mirroring, hub crossovers), legal possession mix constraints,
...
if st.session_state.reports:
    ...
    with tab2:
        ...
        if os.path.exists(res_file):
            ...
            for _, row in results_df.iterrows():
                ...
                ov = row["overrun_days"]
                color = "#10B981" if ov == 0 else "#EF4444"
                ...
                # Add planned milestone marker with NebulaX Navy Blue
                fig.add_trace(
                    go.Scatter(
                        x=[pd.to_datetime(row["planned_completion_date"])],
                        y=[cid],
                        mode="markers",
                        marker=dict(symbol="line-ns", size=18, color="#1E3A8A", line=dict(width=3, color="#1E3A8A")),
    ...
    with tab3:
        ...
        if os.path.exists(occ_file):
            ...
            fig.update_layout(
                height=650,
                title=f"Possession Density by Location & Week (Scenario {selected_sc})",
                font=dict(family="sans-serif", color="#1E293B"),
            )
            st.plotly_chart(fig, width="stretch")
