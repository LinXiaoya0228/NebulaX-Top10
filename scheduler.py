...
class ScenarioScheduler:
    ...
    def _solve_with_cpsat(
        ...
        for cid, contract in dm.contracts.items():
            ...
            w_sum = sum(
                tier_weight * (1.0 + (0.3 if a.activity_priority == 1 else (0.2 if a.activity_priority == 2 else 0.0)))
                for a in c_acts
            )
            # True official unit cost per week of overrun = 7 * w_sum
            # Integer scaled by 10 to preserve decimal precision
            unit_cost = int(round(7 * w_sum * 10))
